...
const Search = () => {
  ...
  const itemsList = () => {
    return itemsData.map((e) => (
      <div className="z-40 " key={e.id}>
        <Song title={e.name} image={e.album.images[0].url} artist={e.artists[0].name}/>
      </div>
    ));
  };
    ...
    <div>
      {searching ? (
        <div className="">
          ...
          >
            <IonIcon name="close-circle-outline" size="20"/>
          </button>
          <div className="bg-black/50 inset-0 absolute z-20">
            ...
            <div className="flex flex-col bg-white relative z-30 h-[59vh] overflow-y-scroll">{itemsList()}</div>
          </div>
        </div>
      ) : (
        <div  onClick={() => {
